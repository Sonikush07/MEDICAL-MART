...
function signUp() {
    if (isset($_POST['singUp'])) {
        ...
        } elseif (!preg_match("/^([a-z0-9\+\-]+)(\.[a-z0-9\+\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$/ix", $email)) {
            setSessionMessage("signup_err_email");
            post_redirect("signUp.php");
        } elseif (!preg_match('/^(?=.\d)(?=.[A-Za-z])[0-9A-Za-z!@#$%]{8,30}$/', $passwd)) {
            setSessionMessage("signup_err_password");
            post_redirect("signUp.php");
        }
